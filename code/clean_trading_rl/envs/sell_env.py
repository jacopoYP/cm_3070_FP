...
class SellEnv:
    ...
    #     """
    #     Dense reward that sums to the trade gross return:
    #     HOLD reward = (p[t+1] - p[t]) / entry_price
    #     Terminal reward:
    #     -exit transaction cost only (exit action causes it)

    #     This shaping avoids myopic "sell asap" behavior and matches the objective.
    #     """
    #     if self.ep is None:
    ...
    #         return self._terminal_state(), -float(self.tc), True, info
    ...
    #     # 3) HOLD (or SELL not allowed yet): dense PnL reward
    #     p0 = float(self.p[t])
    #     p1 = float(self.p[t + 1])

    #     # IMPORTANT: denom is entry_price, not p0 (makes sum == gross)
    #     step_r = (p1 - p0) / (entry_price + 1e-12)

    #     self.ep.t = t + 1
    #     self.ep.bars_held += 1
    #     return self._get_state(), float(step_r), False, info
