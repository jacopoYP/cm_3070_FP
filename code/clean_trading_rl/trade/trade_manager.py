...
#       - Exit uses time-stop (sell_horizon) or end-of-data
#       - Produces entry_indices for SellAgent training

#     Phase B (later, optional):
#       - If sell_agent is provided, it can close positions early
#     """
...
#         # If SellAgent was trained on raw features only, return base.
#         # (Keeps compatibility if you ever re-train with include_pos_features=False)
#         expected_dim = int(getattr(self.sell_agent.cfg, "state_dim", base.shape[0]))
#         if expected_dim == base.shape[0]:
#             return base

#         # Position-derived features (must match SellEnv)
#         entry_idx = int(self._pos.entry_idx)
#         entry_price = float(self._pos.entry_price)
#         price_now = float(self.prices[t])

#         # same "last allowed" logic as SellEnv
#         seg_end = self._segment_end(entry_idx)
#         last_allowed = int(min(entry_idx + int(self.trade_cfg.sell_horizon), seg_end, len(self.prices) - 1))

#         hold = int(t - entry_idx)

#         # Effective decision window (matches last_allowed clamp)
#         eff_horizon = max(1, int(last_allowed - entry_idx))

#         unreal = (price_now / (entry_price + 1e-12)) - 1.0  # exactly matches typical "gross-1"
#         # time_frac = float(min(1.0, hold / eff_horizon))

#         # remaining = float(max(0, last_allowed - t))
#         # remaining_frac = float(min(1.0, remaining / eff_horizon))
...
#         # safety check (fail fast if mismatch)
...
#     #         seg_end = self._segment_end(t)

#     #         # If in position and we're at the last bar of the segment, force close
#     #         if self._pos is not None and t >= seg_end:
#     #             self._close(t, price, forced=True, meta={"reason": "segment_end"})


#     #         # cooldown tick (always)
#     #         if self._cooldown > 0:
#     #             self._cooldown -= 1

#     #         # if in position: check exits
#     #         if self._pos is not None:
#     #             hold = t - self._pos.entry_idx

#     #             # time stop (always available)
#     #             if hold >= int(self.trade_cfg.sell_horizon):
#     #                 self._close(t, price, forced=True, meta={"reason": "time"})
#     #             else:
#     #                 # optional SellAgent exit
#     #                 if self.sell_agent is not None and hold >= int(self.trade_cfg.min_hold_bars):
#     #                     q_sell = self.sell_agent.q_values(s)
#     #                     action = int(np.argmax(q_sell))  # 0=HOLD, 1=SELL (expected)
#     #                     if action == 1:
#     #                         self._close(t, price, forced=False, meta={"reason": "sell_agent"})

#     #         # if flat: check entry
#     #         # if self._pos is None and self._cooldown == 0 and self._trend_ok(t):
#     #         if self._pos is None and self._cooldown == 0 and self._trend_ok(t):
#     #             seg_end = self._segment_end(t)
#     #             latest_entry = seg_end - int(self.trade_cfg.sell_horizon)

#     #             # Don't open if we can't complete the horizon inside this segment
#     #             if t > latest_entry:
#     #                 continue

#     #             q_buy = self.buy_agent.q_values(s)
#     #             conf = self._buy_confidence(q_buy)

#     #             if conf >= float(self.trade_cfg.buy_min_confidence):
...
#             # 2) exits (time stop or sell agent)
#             # if self._pos is not None:
#             if self._pos is not None:
#                 entry_idx = self._pos.entry_idx
#                 last_allowed = self._last_allowed_exit(entry_idx)
#                 hold = t - entry_idx

#                 if t >= last_allowed:
#                     reason = "time" if (entry_idx + self.trade_cfg.sell_horizon) <= last_allowed else "segment_end"
#                     self._close(t, price, forced=True, meta={"reason": reason})
#                 else:
#                     # sell agent logic here

#                     # segment boundary: close exactly at seg_end
#                     if t == seg_end:
#                         self._close(t, price, forced=True, meta={"reason": "segment_end"})

#                     elif hold >= int(self.trade_cfg.sell_horizon):
#                         self._close(t, price, forced=True, meta={"reason": "time"})

#                     elif self.sell_agent is not None and hold >= int(self.trade_cfg.min_hold_bars):
...
#                             SELL_MIN_CONF = float(getattr(self.trade_cfg, "sell_min_confidence", 0.55))
#                             # SELL_MIN_MARGIN = float(getattr(self.trade_cfg, "sell_min_margin", 0.00))

#                             unreal = (price - self._pos.entry_price) / (self._pos.entry_price + 1e-12)

#                             TP = float(getattr(self.trade_cfg, "sell_min_unreal_profit", 0.01))  # +1%
#                             SL = float(getattr(self.trade_cfg, "sell_max_unreal_loss", -0.12))   # -12%

#                             # if action == 1 and sell_conf >= SELL_MIN_CONF and margin >= SELL_MIN_MARGIN:
...
#                         # Expect at least 2 actions: [HOLD, BUY]
...
#         # IMPORTANT: net_return should match the equity convention.
#         # Since we apply tc at entry and tc at exit, the exact net multiplier is:
#         # (1 - tc)^2 * (1 + gross)
#         # but entry tc was applied at _open(), and exit tc applied here, so for reporting:
...
#         if min_gap is None:
#             # sensible default: avoid overlapping sell episodes
#             # (keep >= min_hold or half horizon; tune as you like)
...
#                 continue  # segment too short for a full horizon

#             # score all eligible bars in this segment
...
#                 # IMPORTANT: assumes action 0=HOLD, 1=BUY
#                 q0 = float(q_buy[0])
#                 q1 = float(q_buy[1])
#                 margin = q1 - q0

#                 if use_confidence_score:
#                     score = float(self._buy_confidence(q_buy))
#                 else:
#                     score = float(margin)
